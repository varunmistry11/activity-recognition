...
def convertQueueToList(queue):
    coordinateList = []
    for keypoints in queue:
        for i in range(len(keypoints)):
            coordinateList.append(keypoints[i])
    return coordinateList

def getUsefulRawData(keypoints):
    useful_keypoints = []
    for i in range(3, 42):
        if i % 3 == 0:
            useful_keypoints.append(keypoints[i] / 1280)
        if i % 3 == 1:
            useful_keypoints.append(keypoints[i] / 720)
    return useful_keypoints
            ...
            for row in reader:
                if firstRow :
                    ...
                    for frameNumber in range(startFrameNum, endFrameNum + 1):
                        ...
                        if actionLabel == 'pushing' or actionLabel == 'pushing2' or actionLabel == 'snatching' or actionLabel == 'snatching2':
                            #print(actionLabel, videoNum, frameNumber, 'has ', len(data["people"]), 'people')
                            ...
                            if len(data["people"]) > 0:
                                ...
                                if len(previousData) > 0:
                                    ...
                                    if len(data["people"]) == len(previousData["people"]):
                                        ...
                                        if len(data["people"]) == 1:
                                            ...
                                            listOfQueues[0].append(getUsefulRawData(data["people"][0]["pose_keypoints"]))
                                        else:
                                            #print("current has multiple people")
                                            for personIndex in range(len(listOfQueues)):
                                                listOfQueues[personIndex].append(getUsefulRawData(data["people"][similarityData[str(frameNumber)][personIndex]]["pose_keypoints"]))
                                        ...
                                        if len(previousData["people"]) == 1:
                                            ...
                                            for previousPersonIndex in range(len(similarityData[str(frameNumber - 1)])):
                                                ...
                                                listOfQueues[previousPersonIndex].append(getUsefulRawData(data["people"][similarityData[str(frameNumber)][previousPersonIndex]]["pose_keypoints"]))
                                            ...
                                            for index in currentFrameNewIndices:
                                                listOfQueues.append([])
                                                listOfQueues[len(listOfQueues) - 1].append(getUsefulRawData(data["people"][index]["pose_keypoints"]))
                                ...
                                if len(listOfQueues) > 0:
                                    for personQueue in listOfQueues:
                                        ...
                                        if len(personQueue) == N:
                                            #print("Queue full, appending to list")
                                            inputList.append(convertQueueToList(personQueue))
                                            labelList.append(labelToNum[actionLabel])
                                            personQueue.pop(0)
                                    #print("number of queues", len(listOfQueues))
                            ...
                            if len(queue) == (N):
                                ...
                                inputList.append(convertQueueToList(queue))
                                labelList.append(labelToNum[actionLabel])
                                queue.pop(0)
                            if len(data["people"]) > 0:
                                queue.append(getUsefulRawData(data["people"][0]["pose_keypoints"]))
                            else:
                                #print(actionLabel, videoNum, frameNumber, " has no people")
                                '''
                                Since current frame has no person in it due to elimination,
...
def printTrainTestClasses(y_train, y_test):
    ...
    for y in np.nditer(y_train):
        if y == 1:
            ...
            pushing2_train = pushing2_train + 1
        elif y == 5:
            running_train = running_train + 1
        elif y == 6:
            snatching_train = snatching_train + 1
        elif y == 7:
            snatching2_train = snatching2_train + 1
        elif y == 8:
    ...
    print("Pushing2 class has", pushing2_train, "tuples")
    print("Running class has", running_train, "tuples")
    print("Snatching class has", snatching_train, "tuples")
    print("Snatching2 class has", snatching2_train, "tuples")
    ...
    for y in np.nditer(y_test):
        if y == 1:
            ...
            pushing2_test = pushing2_test + 1
        elif y == 5:
            running_test = running_test + 1
        elif y == 6:
            snatching_test = snatching_test + 1
        elif y == 7:
            snatching2_test = snatching2_test + 1
        elif y == 8:
    ...
    print("Pushing2 class has", pushing2_test, "tuples")
    print("Running class has", running_test, "tuples")
    print("Snatching class has", snatching_test, "tuples")
    print("Snatching2 class has", snatching2_test, "tuples")
    ...
    pushing2 = pushing2_train + pushing2_test
    running = running_train + running_test
    snatching = snatching_train + snatching_test
    snatching2 = snatching2_train + snatching2_test
    vaulting = vaulting_train + vaulting_test
    walking = walking_train + walking_test
    total = eating + kicking + pushing + pushing2 + running + snatching + snatching2 + vaulting + walking
    print("\neating", eating)
    print("kicking", kicking)
    print("pushing", pushing)
    print("pushing2", pushing2)
    print("running", running)
    print("snatching", snatching)
    print("snatching2", snatching2)
    print("vaulting", vaulting)
    print("walking", walking)
    print("\nTotal", total)

def createDataCSV_RawData(N):
    inputNumpyArray, labelNumpyArray = createInputNumpyArray(N)
    ...
    for index in range(len(X_train_list)):
        temp_list = []
        for num in X_train_list[index]:
            temp_list.append(num)
        temp_list.append(y_train_list[index])
        train_list.append(temp_list)

    test_list = []
    for index in range(len(X_test_list)):
        temp_list = []
        for num in X_test_list[index]:
            temp_list.append(num)
        temp_list.append(y_test_list[index])
        test_list.append(temp_list)

    csvfile = 'balanced_data/train_' + str(N) + '_tuples.csv'
    ...
    csvfile = 'balanced_data/test_' + str(N) + '_tuples.csv'
    with open(csvfile, 'w') as output:
        writer = csv.writer(output, lineterminator='\n')
        #writer.writerows(train_list)
