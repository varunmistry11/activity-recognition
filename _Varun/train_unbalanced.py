...
def readData(N):
    ...
    for y in test_y_list:
        temp_list.append(classNumToOneHot[y])
        #b.append(classNumToOneHot[y])
    test_y = np.array(temp_list)

    #unbalanced.printTrainTestClasses(a, b)
...
def saveResults(N, config, maxTestAccuracy, averageTestAccuracy):
    with open('unbalanced_data/normal_results/unb_N_' + str(N) + '_NN_' + config + '.json', 'w') as jsonfile:
        new_data = dict()
...
def trainAllNNModels():
    ...
    for N in range(3, 16):
        ...
        for config in allConfigsPossible:
            if not os.path.isfile('./unbalanced_data/normal_results/unb_N_' + str(N) + '_NN_' + getConfigString(config) + '.json'):
