...
def createInputNumpyArray(N):
    ...
    tuplesOfClass['pushing'] = tuplesOfClass['pushing'] + tuplesOfClass['pushing2']
    tuplesOfClass['snatching'] = tuplesOfClass['snatching'] + tuplesOfClass['snatching2']
    for actionLabel in actionLabels:
        if actionLabel != 'pushing2' and actionLabel != 'snatching2' and tuplesOfClass[actionLabel] < minClassTuples:
            minClassTuples = tuplesOfClass[actionLabel]
    print("Min class tuples", minClassTuples)
    '''
    balancedInputList = []
    balancedLabelList = []
...
def getClassNum(oneHotList):
    classNum = 0
    for index in range(len(oneHotList)):
        if oneHotList[index] == 1:
            classNum = index + 1
            break
    return classNum
...
def createDataCSV(N):
    ...
    for index in range(len(X_train_list)):
        ...
        temp_list.append(getClassNum(y_train_list[index]))
    ...
    for index in range(len(X_test_list)):
        ...
        for num in X_test_list[index]:
            temp_list.append(num)
        temp_list.append(getClassNum(y_test_list[index]))
        test_list.append(temp_list)

    csvfile = 'unbalanced_raw_data/train_' + str(N) + '_tuples.csv'
    ...
    csvfile = 'unbalanced_raw_data/test_' + str(N) + '_tuples.csv'
    with open(csvfile, 'w') as output:
        writer = csv.writer(output, lineterminator='\n')
        #writer.writerows(train_list)
...
if __name__ == '__main__':
    ...
    csvfile = 'train_' + str(N) + '_tuples.csv'
    with open(csvfile, 'w') as output:
        writer = csv.writer(output, lineterminator='\n')
        #writer.writerows(train_list)
        for row in train_list:
    ...
    csvfile = 'test_' + str(N) + '_tuples.csv'
    with open(csvfile, 'w') as output:
        writer = csv.writer(output, lineterminator='\n')
        #writer.writerows(train_list)
        for row in test_list:
